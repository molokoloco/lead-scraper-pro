...
async function main() {
  ...
  for (const filename of filesToProcess) {
    ...
    // Skip only if enriched file is newer than input (input regenerated by merge = new campaign)
    if (fs.existsSync(outputPath) && !fs.existsSync(statePath)) {
      const enrichedMtime = fs.statSync(outputPath).mtimeMs;
      const inputMtime = fs.statSync(inputPath).mtimeMs;
      if (enrichedMtime >= inputMtime) {
        console.log(`⏭️  ${filename} déjà enrichi (fichier à jour), skip.`);
        continue;
      }
    }
