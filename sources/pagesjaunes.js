...
async function scrapePage(page, url) {
  ...
  const items = await page.evaluate(() => {
    ...
    return cards.map(card => {
      ...
      if (id) {
        ...
        if (fantomas) {
          phones = [...fantomas.querySelectorAll('.number-contact')]
            .map(el => el.innerText.replace('Tél :', '').replace(/Opposé.*marketing/i, '').trim())
            .filter(Boolean);
        }
      }
      ...
      // Site web : liens externes (pas pagesjaunes, pas tel:, pas mailto:)
      const websiteLink = [...card.querySelectorAll('a[href^="http"]')]
        .find(a => !a.href.includes('pagesjaunes.fr') && !a.href.startsWith('tel:'));
...
async function main() {
  ...
  const csvLines = ['\uFEFFNom;Adresse;Téléphone(s);Catégorie;Activité'];
  ...
  for (const cat of CATEGORIES) {
    ...
    for (let p = 1; p <= MAX_PAGES_PER_CAT; p++) {
      try {
        ...
        for (const item of items) {
          ...
          csvLines.push(`"${item.name}";"${item.address}";"${phoneStr}";"${cat}";"${item.activity}"`);
    ...
    console.log(`${catCount} résultats Pantin sans site`);
